  ...
  describe("SOL Subscription", () => {
    it("aeon vault recipient subscription test", async () => {
      ...
      expect(initialVaultBalance - finalVaultBalance).to.equals(
        plan.chargeAmount.toNumber()
      );
      expect(finalRecipientBalance - initialRecipientBalance).to.equals(
        plan.chargeAmount.toNumber()
      );
      ...
      expect(subscriptionInfo.owner.toString()).to.equals(
        subscriberWallet.publicKey.toString()
      );
      expect(subscriptionInfo.planId.toNumber()).to.equals(1);
      expect(subscriptionInfo.serviceKey.toString()).to.equals(
        service.toString()
      );
