...
describe("Charge Tests", () => {
  ...
  before(async () => {
    ...
    context = await startAnchor(
      ...
      [
        ...
        {
          address: subscriberVaultAta,
          info: {
            lamports: 1 * web3.LAMPORTS_PER_SOL,
            owner: TOKEN_PROGRAM_ID,
            data: subscriberVaultTokenAccountData,
            executable: false,
          },
        },
      ]
  ...
  describe("SOL charge tests", () => {
    ...
    it("should fail charge with inactive subscription", async () => {
      ...
      // unsubscribe
      await program.methods
        .unsubscribe(new anchor.BN(serviceId))
        .accounts({
          ...{
            feePayer,
            subscriber: subscriberWallet.publicKey,
  ...
  it("should charge when period has elapsed", async () => {
    ...
    const planInfos = [
      {
        chargeAmount: new anchor.BN(10 * 10 ** tokenDecimals),
        interval: new anchor.BN(3600), // 3600 seconds,
        tokenMint: mintKeyPair.publicKey, // represents sol,
        recipient: providerVaultAta, // recipient is the provider's vault
      },
    ];
    await createService(
      connection,
    ...
    await program.methods
      .subscribeToken(new anchor.BN(serviceId), new anchor.BN(1))
      .accounts({ ...accounts })
      .signers([subscriberWallet])
      .rpc();
    ...
    // attempt charge
    const initialVaultAtaBalance = (
      await getAccount(connection, subscriberVaultAta)
    ).amount;
    const initialRecipientBalance = (
      await getAccount(connection, plan.recipient)
    ).amount;
    await program.methods
      .chargeToken(new anchor.BN(serviceId))
      .accounts({
        ...{
          signer: feePayer,
          subscriber: subscriberWallet.publicKey,
      ...
      .rpc();
    // verify subscription state and vault balance checks as well as the recipient
    const finalVaultAtaBalance = (
      await getAccount(connection, subscriberVaultAta)
    ).amount;
    const finalRecipientBalance = (await getAccount(connection, plan.recipient))
      .amount;
    const subscriptionInfo = await program.account.subscription.fetch(
      subscriptionKey
    );
    expect(
      (initialVaultAtaBalance - finalVaultAtaBalance).toString()
    ).to.equals(plan.chargeAmount.toString());
    expect(
      (finalRecipientBalance - initialRecipientBalance).toString()
    ).to.equals(plan.chargeAmount.toString());
    expect(subscriptionInfo.lastChargeTs.toString()).to.equals(
      (currentClock.unixTimestamp + BigInt(plan.interval.toString())).toString()
    );
    expect(subscriptionInfo.nextChargeTs.toNumber()).to.equals(
      subscriptionInfo.lastChargeTs.toNumber() + plan.interval.toNumber()
    );
